struct ContinuousProximalMDRKL <: AbstractActorUpdate
    _reparam::Bool
    ...
    function ContinuousProximalMDRKL(
...
function setup(
    ...
    rng::AbstractRNG;
)::UpdateState{ContinuousProximalMDRKL}
    return UpdateState(
        up,
        optim,
        (
...
function update(
    st::UpdateState{ContinuousProximalMDRKL},
    π::AbstractContinuousParameterisedPolicy,
    π_f,                    # policy model
    π_θ,                    # policy model parameters
    π_st,                   # policy model state
    ...
    # @show up._num_md_updates
    # @show next_update == 1
    # @show st._state.current_update
    # println()
    return UpdateState(
        st._update,
        st._optim,
...
function _rkl_ll_grad(
    ...
    ∇π = gradient(π_θ) do π_θ
        ...
        # l = (up._temperature .- up._inv_λ) .* lnπ_t .- up._inv_λ .* lnπ_θ
        # @show mean(l)
        # @show mean(adv)
        scale = ChainRulesCore.ignore_derivatives(
            adv .- (up._temperature .- up._inv_λ) .* lnπ_t .- up._inv_λ .* lnπ_θ
        )
...
function _rkl_reparam_grad(
    ...
    ∇π_θ = gradient(π_θ) do π_θ
        ...
        # TODO: Rederive with loss as τJ to get this surrogate
        loss = adv .- (up._temperature .- up._inv_λ) .* lnπ_t .- up._inv_λ .* lnπ_θ
