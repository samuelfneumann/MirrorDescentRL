...
function get_expr(er::ImageReplay{<:AbstractSequenceReplay}, k, expr)
    if k == :s
        ...
        MLUtils.batchseq([[get_state.(seq); [get_image(er, seq[end].sp)]] for seq in expr], zero(get_state(expr[1][1])))
    elseif k == :sp
...
function Random.rand(er::ImageReplay, batch_size, args...)
    rand(Random.GLOBAL_RNG, er, batch_size, args...)
end

function Random.rand(rng::Random.AbstractRNG, er::ImageReplay, batch_size, args...)
    exp_s_idx, experience = rand(rng, er.replay, batch_size, args...)
